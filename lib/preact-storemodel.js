...
class StoreModel {
    constructor(name, opts) {
        ...
        this.namePluralReal = opts.namePluralReal;
        this.api = opts.api;
        this.opts = opts;
    }
    ...
    updateItem(item) {
        ...
        if (newObject !== currentObject) {
            if (isDevelopment) {
                const differences = diff(currentObject, newObject);
                console.log(`preact-storemodel.updateItem.${this.name}: Diff:`);
    ...
    updateFieldByName({ namePlural = this.namePlural, name = this.name, id, field, value, findBy = 'id' }) {
        if (util.isArray(this[namePlural])) {
        ...
        if (response.status < 400) {
            if (Array.isArray(opt.addData)) {
                opt.addData.forEach((el) => {
                    if (util.isDefined(response[el])) {
                        this.updateKeyValue(el, response[el]);
                    } else if (response.included && util.isDefined(response.included[el])) {
    ...
    async load($id = '', append, $opt = {}) {
        ...
        const url = opt.url || this.apiLoadUrl || util.getNestedValue(this, 'api.load.url');
        const qf = opt.skipFilter ? {} : (opt.query || this.queryFilter);
        // TODO: Should accept query also when id is set.
        // const queryFilter = id ? {} : { ...qf };
        const queryFilter = { ...qf };
    ...
    async saveField(id, field, value, updateMemory, opt = {}) {
        ...
        const body = {
            ...
            event: this.getEventInfo(opt.event),
        };
        if (opt.query) {
    ...
    async save(data, id, event) {
        const objId = id || data.id || this[this.name].id;
        const url = util.getNestedValue(this, 'api.save.url');

        const response = await util.fetchApi(`${url}${objId}`,
    ...
    async delete(id, field = 'id', data = {}, event) {
